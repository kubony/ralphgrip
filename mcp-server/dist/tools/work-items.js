...
async function getNextPosition(projectId, parentId) {
...
async function resolveStatusId(projectId, statusName) {
...
async function resolveTrackerId(projectId, trackerName) {
    const { data, error } = await supabase
        .from('trackers')
        .select('id')
        .eq('project_id', projectId)
...
async function resolveWorkItemByNumber(projectId, number) {
    const { data, error } = await supabase
        .from('work_items')
        .select('id')
        .eq('project_id', projectId)
...
export function registerWorkItemTools(server) {
    // ── create_task ──
    server.tool('create_task', 'Create a new task (work item) in the AgentGrip project. Use tracker and status names (e.g., "Task", "To Do") — UUIDs are resolved automatically.', {
        ...
        try {
            ...
            return {
                content: [{ type: 'text', text: `Failed after ${MAX_POSITION_INSERT_ATTEMPTS} attempts: ${lastError?.message}` }],
            };
        }
        catch (err) {
    ...
    // ── update_task ──
    server.tool('update_task', 'Update an existing task by its number (e.g., 42). You can change status, title, description, priority, or assignee.', {
        ...
        try {
            ...
            const updates = {
                ai_metadata: { model: 'mcp-server', last_action: 'update', updated_fields: [], agent_id: actorIds.agentId },
            };
            const updatedFields = [];
            if (args.status) {
            ...
            ;
            updates.ai_metadata.updated_fields = updatedFields;
    ...
    // ── list_tasks ──
    server.tool('list_tasks', 'List tasks in the project. Filter by status, tracker, or assignee.', {
    ...
    // ── get_task ──
    server.tool('get_task', 'Get detailed information about a single task by its number.', {
        number: z.number().describe('Work item number (e.g., 42)'),
    }, async (args) => {
        const projectId = getProjectId();
        const { data, error } = await supabase
    ...
    // ── add_comment ──
    server.tool('add_comment', 'Add a comment to a task by its number. If running as an agent (MADSPEED_AGENT_ID set), the comment is attributed to the agent.', {
        number: z.number().describe('Work item number (e.g., 42)'),
        ...
        try {
            ...
            const { data, error } = await supabase
                ...
                .single();
            if (error) {
                return { content: [{ type: 'text', text: `Error adding comment: ${error.message}` }] };
            }
            return {
                content: [{
                        type: 'text',
