...
import { createTestSupabase, createTestProject, createTestAgent, cleanupTestProject, } from '../test/helpers.js';
let supabase;
let project;
let agent;
beforeAll(async () => {
...
describe('Work Items — Supabase DB 연동', () => {
    ...
    it('get_task: 관계 데이터 (tracker, status, assignee) 포함', async () => {
      ...
      `)
        ...
        expect(data.tracker.name).toBe(project.trackerName);
        expect(data.status).toBeDefined();
    });
});
//# sourceMappingURL=work-items.integration.test.js.map