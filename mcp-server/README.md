...
| `report_progress` | `number`, `message`, `project_key?`, `set_in_progress?`(기본 true), `git?` | 진행 보고 → **In Progress** 전이 + 댓글 |
| `report_blocker` | `number`, `blocker`, `project_key?`, `git?` | 블로커 보고 → **Issue** 전이 + 댓글 |
| `mark_resolved` | `number`, `summary`, `project_key?`, `git?` | 완료 보고 → **Resolved** 전이 + 댓글 |
...
| `create_project` | `name`(필수), `key`(필수, 2–10자 영문 대문자), `project_type`(필수, `issue`\|`requirement`), `description?`, `repo_url?` | 새 프로젝트 생성. owner는 에이전트 소유자로 지정되고 DB 트리거가 멤버십·트래커·상태를 자동 생성. `repo_url` 전달 시 레포 1:1 아이덴티티로 `settings.repo.url`에 저장. **owned/global 에이전트만** 호출 가능 |

### Work Items

| Tool | 핵심 인자 | Description |
|------|-----------|-------------|
| `create_task` | `title`(필수), `description?`, `tracker?`(기본 `"Task"`), `status?`(기본 `"Open"`), `priority?`(0–4), `assignee_id?`, `assign_to_self?`, `parent_number?`, `project_key?`, `git?` | 작업 생성 (이름 기반 해석) |
...
사람이 웹 UI에서 "이 태스크를 어느 레포/브랜치/워크트리에서 작업 중인지" 볼 수 있도록,
`create_task` · `report_progress` · `report_blocker` · `mark_resolved` 호출 시 `git` 인자를 함께 넘깁니다.

```json
"git": {
  "repo_url": "https://github.com/kubony/ralphgrip",
  "branch": "feat/git-context-backend",
  "worktree": "/path/to/worktree",
  "commit": "d94e435"
}
```

- 모든 필드는 optional입니다. `worktree`는 메인 체크아웃이면 생략할 수 있습니다.
- `repo_url`은 레포 루트 `.ralphgrip.json`의 `repo_url`을, `branch`/`commit`은 `git branch --show-current` / `git rev-parse --short HEAD`로 수집합니다.
- 전달하면 `work_items.git_context`에 **교체(merge 아님)**로 저장되고, 서버가 `updated_at`(ISO8601)을 기록 시점에 추가합니다. 전달하지 않으면 기존 값이 유지됩니다.
...
### 레포-프로젝트 고정 (1 repo = 1 project)

한 레포에서 여러 Claude Code 세션이 동시에 일하면, 에이전트마다 다른 RalphGrip 프로젝트에
태스크를 만들어 **작업이 여러 프로젝트로 흩어지는** 문제가 생깁니다. 이를 막기 위해
"이 레포는 항상 이 프로젝트 하나로만 보고한다"를 **선언 + 지시 + 훅 강제** 3중으로 고정합니다.
자기 레포에도 그대로 복사해 쓸 수 있습니다.

1. 레포 루트에 `.ralphgrip.json`을 만들어 고정할 프로젝트 키를 선언한다 (이 레포는 `RG`):

   ```json
   {"project_key": "RG"}
   ```

2. 이 레포의 `.claude/hooks/enforce-project-key.sh`를 자기 레포 같은 위치로 복사하고 실행 권한을 준다.
   이 훅은 PreToolUse에서 `mcp__ralphgrip__*` 호출을 가로채,
   `.ralphgrip.json`의 `project_key`와 다르거나 `project_key`가 **누락된** 호출을 `deny`로 차단하고
   고정 키로 다시 호출하도록 지시한다.

   ```bash
   chmod +x .claude/hooks/enforce-project-key.sh
   ```

   > 예외 툴(검사하지 않고 통과): `whoami`, `list_projects`, `search`, `delete_link`, `create_project`.
   > 이들은 기존 프로젝트의 `project_key`를 인자로 받지 않기 때문이다
   > (`create_project`는 새 프로젝트를 만드는 툴). `create_link`은 source 쪽 `project_key`만 검사하고
   > 크로스 프로젝트용 `target_project_key`는 검사하지 않는다.
   > `.ralphgrip.json`이 없거나 파싱에 실패하면 훅은 아무것도 고정하지 않고 통과시킨다.

3. `.claude/settings.json`의 `PreToolUse`에 훅을 등록한다:
   ...
   ```json
   {
     "hooks": {
       "PreToolUse": [
         {
           "matcher": "mcp__ralphgrip__.*",
           "hooks": [{"type": "command", "command": "${CLAUDE_PROJECT_DIR}/.claude/hooks/enforce-project-key.sh", "timeout": 10}]
         }
       ]
     }
   }
...
4. `CLAUDE.md`에 "이 레포의 RalphGrip 프로젝트는 `.ralphgrip.json`에 고정된 키다.
   모든 RalphGrip 툴 호출에 그 `project_key`를 명시한다"는 규칙을 넣어 에이전트가 사전에 인지하게 한다.
   선언(`.ralphgrip.json`) · 지시(`CLAUDE.md`) · 훅 강제(`enforce-project-key.sh`)가 함께 동작해,
   에이전트가 규칙을 잊어도 훅이 마지막 방어선이 된다.
