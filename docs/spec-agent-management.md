...
RalphGrip에서 AI 에이전트를 이슈(Work Item)의 담당자로 할당하려 하면 "저장에 실패했습니다" 에러가 발생한다. 현재 `work_items.assignee_id`는 `profiles` 테이블을 FK로 참조하고 있어, 사람이 아닌 에이전트를 담당자로 저장할 수 없다.

에이전트는 RalphGrip의 핵심 기능으로, Claude/GPT 등 LLM을 사용자의 구독 정보(API 키)로 CLI 세션을 열어 컨텍스트를 전달하고 업무를 수행하는 봇이다. OpenClaw(오픈소스 에이전트 플랫폼)를 런타임으로 활용할 수도 있다.
...
- **RalphGrip / RalphGrip E2E 실행 원칙 문서화**
...
3. OpenClaw 설치를 RalphGrip UI에서 원클릭으로 할 수 있는지, 아니면 가이드만 제공할지?
