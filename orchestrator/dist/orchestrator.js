...
export class Orchestrator {
    ...
    constructor(loader, tracker, workspaceManager) {
        this.loader = loader;
    ...
    async poll() {
        ...
        try {
            ...
            const candidates = issues.filter((issue) => {
                ...
                if (retry && now < retry.nextRetryAt)
                    return false;
                return true;
            });
    ...
    async dispatch(issue) {
        ...
        try {
            ...
            if (result.success) {
                ...
                await this.tracker.addComment(issue.id, `🤖 **AgentGrip Orchestrator** — 작업 완료 (attempt ${attempt})\n\n` +
                ...
                // Add failure comment
                await this.tracker.addComment(issue.id, `🤖 **AgentGrip Orchestrator** — 작업 실패 (attempt ${attempt})\n\n` +
                    `Error: ${result.error}\n` +
                    `Duration: ${Math.round(result.durationMs / 1000)}s`);
    ...
    async handleContinuation(issue, entry, prevResult) {
        ...
        if (contResult.success) {
            await this.tracker.addComment(issue.id, `🤖 **AgentGrip Orchestrator** — continuation 완료\n\n` +
    ...
    scheduleRetry(issueId, identifier, attempt) {
        const backoffMs = Math.min(10_000 * Math.pow(2, attempt - 1), this.config.agent.max_retry_backoff_ms);
