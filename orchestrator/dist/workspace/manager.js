...
export class WorkspaceManager {
    ...
    async prepareForRun(identifier) {
    ...
    async cleanupAfterRun(identifier) {
    ...
    runHook(name, script, cwd, fatal) {
        return new Promise((resolve, reject) => {
            ...
            const child = exec(`sh -lc ${JSON.stringify(script)}`, {
                ...
                if (stdout)
                    log.debug(`[${name}] stdout`, { output: stdout.trim() });
                if (stderr)
                    log.debug(`[${name}] stderr`, { output: stderr.trim() });
                if (error) {
