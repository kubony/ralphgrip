...
RalphGrip은 사람이 웹 UI에서 에이전트의 진행 상황을 실시간으로 지켜보는 시스템이다.
보고는 **시작 → 진행 중 → 종료** 3단계로 계속한다:

- **시작**: 작업에 착수하면 먼저 `report_progress`로 무엇을 할지 선언 (→ In Progress)
- **진행 중**: 마일스톤마다 `report_progress`로 중간 보고. 파일 수정 5회 누적 시 hook이 중간 보고를 요구하며, 요구를 받으면 즉시 보고 후 작업 계속
- **종료**: 완료면 `mark_resolved`, 미완이면 `report_progress`로 현재 상태 기록
